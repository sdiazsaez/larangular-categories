...
namespace Larangular\Categories\Traits;
...
trait Categorizable {
     ...
     * @return void
     */
    abstract public static function saved($callback): void;
     ...
     * @return void
     */
    abstract public static function deleted($callback): void;
    ...
    abstract public function morphToMany($related, $name, $table = null, $foreignPivotKey = null,
        $relatedPivotKey = null, $parentKey = null, $relatedKey = null, $inverse = false): MorphToMany;
    ...
    public function categories(): MorphToMany {
        return $this->morphToMany($this->categorizable_getTable('categories'), 'categorizable',
            $this->categorizable_getTable('categorizables'), 'categorizable_id', 'category_id')
                    ->withTimestamps();
    }
     ...
     *
     * @param int|string|array|\ArrayAccess|\Larangular\Categories\Models\Category $categories
     *
     * @return void
     */
    ...
    public function scopeWithAllCategories(Builder $builder, $categories): Builder {
        ...
        collect($categories)->each(function ($category) use ($builder) {
            $builder->whereHas($this->categorizable_getTable('categories'),
    ...
    public function scopeWithAnyCategories(Builder $builder, $categories): Builder {
        ...
        return $builder->whereHas($this->categorizable_getTable('categories'),
            function (Builder $builder) use ($categories) {
    ...
    public function scopeWithoutCategories(Builder $builder, $categories): Builder {
        ...
        return $builder->whereDoesntHave($this->categorizable_getTable('categories'),
            function (Builder $builder) use ($categories) {
                $builder->whereIn('id', $categories);
            });
    }
    ...
    public function scopeWithoutAnyCategories(Builder $builder): Builder {
        return $builder->doesntHave($this->categorizable_getTable('categories'));
    }
    ...
    public function detachCategories($categories = null) {
        $categories = !is_null($categories)
            ? $this->prepareCategoryIds($categories)
            : null;
    ...
    protected function prepareCategoryIds($categories): array {
        ...
        if (is_string($categories) || (is_array($categories) && is_string(Arr::first($categories)))) {
            $categories = Category::whereIn('slug', (array)$categories)
                                  ->get()
                                  ->pluck('id');
        }
    ...
    private function categorizable_getTable(string $name) {
        return app('larangular.categories.installable')->getName($name);
    }
}
